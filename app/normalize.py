...
# ------------------------------------------------- pemindaian JSON tak dikenal
_ATTACKER_HINTS = ("attacker", "source", "src", "remote", "client", "offender", "malicious")


def deep_find_ips(payload: Any, max_depth: int = 8) -> list[str]:
    """Telusuri struktur JSON apa pun dan kumpulkan nilai yang valid sebagai IP.

    Dipakai untuk integrasi yang formatnya tidak kita kendalikan (mis. webhook
    FortiDeceptor). Kunci yang mengandung kata 'attacker', 'source', 'src', dan
    sejenisnya diprioritaskan supaya IP milik perangkat pengirim tidak ikut
    terambil ketika payload memuat beberapa alamat sekaligus.
    """
    preferred: list[str] = []
    fallback: list[str] = []
    ...
    def walk(node: Any, path: str = "", depth: int = 0) -> None:
        ...
        if isinstance(node, dict):
            for k, v in node.items():
                # Jalur penuh dibawa, bukan hanya kunci terakhir: pada
                # {"attacker": {"ipv4": ...}} petunjuknya ada di induk.
                walk(v, f"{path}.{str(k).lower()}", depth + 1)
            ...
            if not is_routable(ip):
                return
            bucket = preferred if any(h in path for h in _ATTACKER_HINTS) else fallback
            if ip not in bucket:
                bucket.append(ip)
